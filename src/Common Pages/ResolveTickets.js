import { faBackward, faBell, faCalendarAlt, faHandHolding, faHandHoldingHand, faListUl, faPeopleGroup, faRecycle, faRefresh, faSave, faUserCircle, faUserPen } from "@fortawesome/free-solid-svg-icons";
...
import "./Ticket.css"
import { FaCircle } from "react-icons/fa"; // For unread indicator
...
export default function ResolveTickets({ activeTab }) {
    ...
    const [approveStatus, setApproveStatus] = useState('N');
    ...
    const handleFileChange1 = (event) => {
        const files = Array.from(event.target.files);
        setSelectedFiles1([...selectedFiles1, ...files]); // Append new files
    };
    ...
    const handleSave = () => {
        ...
        setLoading(true);
        axios.post(`${ipaddress}changeMangement/resolveTicket`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const getTicket = () => {
        setLoading(true);
        axios.get(`${ipaddress}changeMangement/getResolvedTickets?cid=${companyid}&bid=${branchId}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
                    ...
                    <div className="ticket-container">
                        ...
                        {ticketData.length > 0 ? (
                            ticketData.map((ticket, index) => (
                                <div
                                    key={index}
                                    className={`ticket-item ${ticket.ticketStatus === "New" ? "unread" : ""}`}
                                    onClick={() => getDataByTicketId(ticket.ticketNo)}
                                >
                                    <FaCircle className={`status-icon ${ticket.ticketStatus === "New" ? "unread-icon" : "read-icon"}`} />
                                    <div className="ticket-info">
                                        <div className="ticket-header">
                                            <strong>{ticket.requester}</strong> → <span>{ticket.assignee}</span> <span>({ticket.ticketNo})</span>
                                        </div>
                                        <div className="ticket-subject">
                                            <strong>{ticket.subject}</strong>
                                            <span style={{ float: 'inline-end', color: 'lightgreen' }}>{ticket.ticketStatus}</span>
                                        </div>
                                        <div className="ticket-meta">
                                            <span className="ticket-priority">{ticket.priority}</span>
                                            <span className="ticket-date">{new Date(ticket.createdDate).toLocaleString()}</span>
                                        </div>
                                    </div>
                                </div>
                            ))
                            ...
                            <hr className="my-2" style={{ borderTop: "1px solid #ccc" }} />
                                                ...
                                                <div className="d-flex justify-content-between align-items-center">
                                                    ...
                                                    <div>
                                                        ...
                                                        {/* Attach File Button */}
                                                        <label htmlFor="attachment3" style={{ fontSize: 14 }} className="btn btn-outline-secondary">
                                                            📎 Attach File
                                                        </label>
                                                            ...
                                                            <Col md={12}>
                                                                <div style={{ display: 'flex', alignItems: 'center' }}>
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        onClick={handleSave}
                                                                        disabled={ticketHdr.ticketStatus === 'New'}
                                                                    >
                                                                        <FontAwesomeIcon
                                                                            icon={faSave}
                                                                            style={{ marginRight: "5px" }}
