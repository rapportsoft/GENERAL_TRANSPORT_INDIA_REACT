...
export default function GeneralContainerInvoice({ activeTab }) {
  ...
  const getSelectedBeforeSearchDataForExBond = (val) => {
    ...
    axios
      .get(
        `${ipaddress}assessment/getGeneralConDeliveryBeforeSaveAssessmentData?cid=${companyid}&bid=${branchId}&val=${val}`,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
          },
  ...
  const getAllCfInvSrvAnxList = async (
    ...
    try {
      const response = await FinanceService.getAllCfInvSrvAnxList(
        companyId,
        branchId,
        assesmentId,
        assesmentLineNo,
  ...
  const searchServices = async (searchvalue, containerSize, containerType) => {
    try {
      const response = await FinanceService.searchServicesForAddService(
        companyid,
        branchId,
        searchvalue,
  ...
  const saveAddService = async (serviceData) => {
    ...
    setLoading(true);
    try {
      const response = await FinanceService.addServiceGeneralConInvoice(
        companyid,
        branchId,
        userId,
        assessmentData,
  ...
  const getAllContainerListOfAssessMentSheet = async (
    ...
    try {
      const response =
        await FinanceService.getAllContainerListOfAssessMentSheetGeneralCon(
          companyId,
          branchId,
          assesmentId,
          profitcentreId,
        ...
        >
          <Row>
            <Col md={5}>
              <FormGroup>
                <label className="forlabel bold-label" htmlFor="sbRequestId">
                  Search by Invoice No / Assessment Id / Job trans Id / Job No /
                  BOE No
                </label>
                <input
                  className="form-control"
