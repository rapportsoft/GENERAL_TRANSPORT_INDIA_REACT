...
export default function GeneralContainerInvoice({ activeTab }) {
    ...
    const handleSave = () => {
        ...
        axios.post(`${ipaddress}assessment/saveContainerWiseGeneralReceivingAssessmentData?cid=${companyid}&bid=${branchId}&user=${userId}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const handleSave1 = () => {
        ...
        axios.post(`${ipaddress}assessment/saveGeneralContWiseDeliveryAssessmentData?cid=${companyid}&bid=${branchId}&user=${userId}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const saveProcess = () => {
        ...
        axios.post(`${ipaddress}assessment/saveContainerWiseGeneralJobInvoiceReceipt?cid=${companyid}&bid=${branchId}&user=${userId}&creditStatus=${assessmentData.creditType}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const saveProcess1 = () => {
        ...
        axios.post(`${ipaddress}assessment/saveGeneralContWiseDeliveryInvoiceReceipt?cid=${companyid}&bid=${branchId}&user=${userId}&creditStatus=${assessmentData.creditType}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const searchExportEmptyContainerGateIn = (id) => {
        setLoading(true);
        axios.get(`${ipaddress}assessment/searchGeneralConInvoiceData?cid=${companyid}&bid=${branchId}&val=${id}&type=${selectedInvoice}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const handlePrint1 = async (type) => {
        ...
        axios.post(`${ipaddress}importinvoiceprint/printContainerWiseGeneralInvoicepdf`, null, {
            headers: {
        ...
        axios.get(`${ipaddress}assessment/getGeneralConDeliveryBeforeSaveAssessmentData?cid=${companyid}&bid=${branchId}&val=${val}`, {
            ...
            .then((response) => {
                ...
                setContainerData(data.map((item) => ({
                    ...
                    checkDate: 'N',
                    invoiceDate: null,
                    lastInvoiceUptoDate: item[23] === null ? null : new Date(new Date(item[23]).setDate(new Date(item[23]).getDate() + 1)),
                    upTariffNo: "",
                    profitcentreId: item[1] || '',
                    grossWt: item[22] ? (item[22] / 1000).toFixed(3) : '',
