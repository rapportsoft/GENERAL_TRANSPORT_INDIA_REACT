...
function ExportInvoice({ activeTab }) {
    ...
    const searchAssesMentSearch = async (searchvalue) => {
        ...
        setLoading(true);
        try {
            const response = await FinanceService.getAssesMentEntriesToSelect(companyid, branchId, searchvalue, selectedInvoiceType, jwtToken);
    ...
    const getSelectedAssesMentSearch = async (profitcentreId, assesMentId, invoiceNo, sbTransId) => {
        ...
        setLoading(true);
        try {
            const response = await FinanceService.getSelectedAssesMentSearch(companyid, branchId, profitcentreId, assesMentId, invoiceNo, sbTransId, selectedInvoiceType, jwtToken);
    ...
    const handleSave = async (assesmentSheet, assesmentData) => {
        ...
        try {
            const response = await FinanceService.saveExportAssesmentContainer(companyid, branchId, userId, selectedInvoiceType, assesmentSheet, filteredData, jwtToken);
            ...
            if (assesmentSheetRec.creditType === 'Y') {
                setPaymentMode([{
                    payMode: 'CREDIT',
                    chequeNo: '',
                    chequeDate: null,
                    bankDetails: '',
                    amount: finaltotalRateWithTax,
                    status: ''
                }]);
                setInvoiceAmt(finaltotalRateWithTax);
                setBalanceAmt(0);
            } else if (assesmentSheetRec.creditType === 'P') {
                setPaymentMode([{
                    payMode: 'ADVANCE',
                    chequeNo: '',
                    chequeDate: null,
    ...
    const resestMainSearch = () => {
        // setMainSearch(initialMainsearch);
    }
    ...
    const searchServices = async (searchvalue, containerSize, containerType) => {
        try {
            const response = await FinanceService.searchServicesForAddService(companyid, branchId, searchvalue, assessmentSheet.commodityCode, containerSize, containerType, assessmentSheet, jwtToken);
    ...
    const saveAddService = async (serviceData) => {
        ...
        setLoading(true);
        try {
            const response = await FinanceService.addServiceExportInvoice(companyid, branchId, userId, assessmentSheet, serviceData, jwtToken);
    ...
    const getAllContainerListOfAssessMentSheet = async (companyId, branchId, assesmentId, profitcentreId) => {
        try {
            const response = await FinanceService.getAllContainerListOfAssessMentSheet(companyId, branchId, assesmentId, profitcentreId, selectedInvoiceType, jwtToken);
    ...
    const getAllContainerDetailsOfAssesmentId = async () => {
        try {
            const response = await FinanceService.getAllContainerDetailsOfAssesmentId(companyid, branchId, assessmentSheet.assesmentId, assessmentSheet.profitcentreId, selectedInvoiceType, jwtToken);
            ...
            if (assessmentSheet.creditType === 'Y') {
                setPaymentMode([{
                    payMode: 'CREDIT',
                    chequeNo: '',
                    chequeDate: null,
                ...
                setInvoiceAmt(finaltotalRateWithTax);
                setBalanceAmt(0);
            } else if (assessmentSheet.creditType === 'P') {
                setPaymentMode([{
                    payMode: 'ADVANCE',
                    chequeNo: '',
                    chequeDate: null,
    ...
    const saveAddServiceContainerWise = async (selectedContainers) => {
        ...
        try {
            const response = await FinanceService.addServiceExportInvoiceServiceWise(companyid, branchId, userId, assessmentSheet, selectedContainers, jwtToken);
    ...
    const handleProcess = async () => {
        ...
        if (assessmentSheet.creditType === 'Y') {
            ...
            if (assessmentSheet.pendingCredit < creditAmt) {
        ...
        if (assessmentSheet.creditType === 'P') {
            const creditRed = paymentMode.filter(item => item.payMode === "ADVANCE");

            const creditAmt = creditRed.reduce((total, item) => total + parseFloat(item.amount), 0);

            if (assessmentSheet.advanceAmount < creditAmt) {
        ...
        try {
            const response = await FinanceService.processExportAssesmentContainer(companyid, branchId, userId, formData, selectedInvoiceType, assessmentSheet.creditType, jwtToken);
...
<Col xs={12} md={6}>
  ...
  <Col xs={12} className="d-flex flex-wrap justify-content-center gap-2">
                            ...
                            <button
                                ...
                                style={{ marginRight: 10 }}
                                id="submitbtn2"
                                onClick={() => { handleClear('main'); resestMainSearch(); }}
                            >
                            ...
                            <Row className='text-center'>
                                <Col>
                                    <button
                                        ...
                                        disabled={balanceAmt === '' || balanceAmt === 0 || assessmentSheet.invoiceNo !== ''}
                                    >
                                        <FontAwesomeIcon
                                            icon={faAdd}
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <Input
                                                            ...
                                                            onChange={(e) => handlePaymentModeChange(e, index)}
                                                            style={{ width: 180 }}
                                                            disabled={item.payMode === '' || item.payMode === 'CASH' || item.payMode === 'TDS' || assessmentSheet.invoiceNo !== ''}
                                                        />
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <Input
                                                            ...
                                                            name='amount'
                                                            value={item.amount}
                                                            disabled={item.payMode === '' || item.payMode === 'TDS' || assessmentSheet.invoiceNo !== ''}
                                                            onChange={(e) => handlePaymentModeChange(e, index)}
                                                            style={{ width: 180 }}
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <Input
                                                            ...
                                                            name='payMode'
                                                            value={item.payMode}
                                                            disabled={assessmentSheet.invoiceNo !== '' || item.payMode === 'CREDIT' || item.payMode === 'ADVANCE'}
                                                            onChange={(e) => handlePaymentModeChange(e, index)}
                                                            style={{ width: 150 }}
                                                        >
                                                            <option value=""></option>
                                                            {assessmentSheet.creditType === "Y" && (<option value="CREDIT">CREDIT</option>)}
                                                            {assessmentSheet.creditType === "P" && (<option value="ADVANCE">ADVANCE</option>)}

                                                            {assessmentSheet.creditType === "P" && (<option value="TDS">TDS</option>)}
                                                    ...
                                                    <td>
                                                        <Input
                                                            ...
                                                            onChange={(e) => handlePaymentModeChange(e, index)}
                                                            style={{ width: 180 }}
                                                            disabled={item.payMode === '' || item.payMode === 'ADVANCE' || item.payMode === 'CASH' || item.payMode === 'TDS' || item.payMode === 'CREDIT' || assessmentSheet.invoiceNo !== ''}
                                                        />
                                                    </td>
                                                    <td>
                                                        <div style={{ position: 'relative', width: 210 }}>
                                                            <DatePicker
                                                                ...
                                                                portalId="datepicker-portal4"
                                                                disabled={item.payMode === '' || item.payMode === 'ADVANCE' || item.payMode === 'CREDIT' || item.payMode === 'CASH' || item.payMode === 'TDS' || assessmentSheet.invoiceNo !== ''}
                                                                popperPlacement="top-start"
                                                                id="chequeDate" // Unique identifier
                                                                name="chequeDate" // Name of the field
                                                                dateFormat="dd/MM/yyyy HH:mm" // Display format for the date and time
                                                    ...
                                                    <td>
                                                        <Input
                                                            ...
                                                            name='amount'
                                                            value={item.amount}
                                                            disabled={item.payMode === '' || item.payMode === 'CREDIT' || item.payMode === 'TDS' || assessmentSheet.invoiceNo !== ''}
                                                            onChange={(e) => handlePaymentModeChange(e, index)}
                                                            style={{ width: 180 }}
