...
export default function GeneralContainerProforma({ activeTab }) {
    ...
    const saveAddService = async (serviceData) => {
        ...
        setLoading(true);
        try {
            const response = await FinanceService.addServiceGeneralConInvoicePro(companyid, branchId, userId, assessmentData, serviceData, jwtToken, selectedInvoice);
    ...
    const getAllContainerListOfAssessMentSheet = async (companyId, branchId, assesmentId, profitcentreId) => {
        try {
            const response = await FinanceService.getAllContainerListOfAssessMentSheetGeneralConPro(companyId, branchId, assesmentId, profitcentreId, jwtToken);
