...
export default function GeneralProforma({ activeTab }) {
    ...
    const handleSave = () => {
        ...
        axios.post(`${ipaddress}assessment/saveGeneralReceivingAssessmentDataPro?cid=${companyid}&bid=${branchId}&user=${userId}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
...
console.log(' containerData ', containerData);
    ...
    const handleSave1 = () => {
        ...
        axios.post(`${ipaddress}assessment/saveGeneralDeliveryAssessmentDataPro?cid=${companyid}&bid=${branchId}&user=${userId}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const saveProcess = () => {
        ...
        axios.post(`${ipaddress}assessment/saveGeneralJobInvoiceReceiptPro?cid=${companyid}&bid=${branchId}&user=${userId}&creditStatus=${assessmentData.creditType}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const saveProcess1 = () => {
        ...
        axios.post(`${ipaddress}assessment/saveGeneralDeliveryInvoiceReceiptPro?cid=${companyid}&bid=${branchId}&user=${userId}&creditStatus=${assessmentData.creditType}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const searchExportEmptyContainerGateIn = (id) => {
        setLoading(true);
        axios.get(`${ipaddress}assessment/searchGeneralInvoiceDataPro?cid=${companyid}&bid=${branchId}&val=${id}&type=${selectedInvoice}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const getSelectedInvoiceData = (assId, invId) => {
        axios.get(`${ipaddress}assessment/getSelectedBondNOCInvoiceDataPro?cid=${companyid}&bid=${branchId}&assId=${assId}&invId=${invId}&type=${selectedInvoice}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const handlePrint1 = async (type) => {
        ...
        axios.post(`${ipaddress}importinvoiceproformaprint/printGeneralInvoicepdf/${companyid}/${branchId}/${invoiceNo}/${assesmentId}/${igmTransId}/${selectedInvoice}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const getAllContainerDetailsOfAssesmentId = () => {
        try {
            ...
            axios.get(`${ipaddress}assessment/getDataByAfterAssessmentForBondPro?cid=${companyid}&bid=${branchId}&id=${assessmentData.assesmentId}&partyId=${partyId}&creditType=${assessmentData.creditType}&type=${selectedInvoice}`, {
                headers: {
                    Authorization: `Bearer ${jwtToken}`
                }
            })
    ...
    const saveAddServiceContainerWise = async (selectedContainers) => {
        ...
        try {
            const response = await FinanceService.addServiceExportInvoiceServiceWisePro(companyid, branchId, userId, assessmentData, Cfinvsrvanx, jwtToken, selectedInvoice);
                ...
                <Row>
                    ...
                    {selectedInvoice === "noc" && (
                        <Col md={4}>
                            <FormGroup>
                                ...
                                <Select
                                    value={searchval ? { value: searchval, label: searchval } : null}
                                    onChange={handleSearchChange}
                                    onInputChange={getSearchData}
                                    options={searchData}
                                    placeholder="Search By BE No"
                                    isClearable
                    ...
                    {selectedInvoice === "exbond" && (
                        <Col md={4}>
                            <FormGroup>
                                <label className="forlabel bold-label" htmlFor="sbRequestId">
                                    Search BE No / Delivery Id
                                </label>
                                <Select
                                    value={selectExBoeData ? { value: selectExBoeData, label: selectExBoeData } : null}
                                    onChange={handleExBondBoeSearchChange}
                                    onInputChange={getExBoeSearchData}
                                    options={exBoeData}
                                    placeholder="Search BE No / Delivery Id"
                                    isClearable
                                    menuPortalTarget={document.body} // Renders dropdown in the document body to avoid clipping
                                    menuPosition="fixed" // Sets the dropdown menu position to fixed    
                                    id="searchval"
