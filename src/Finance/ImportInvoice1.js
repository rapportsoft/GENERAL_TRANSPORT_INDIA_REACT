...
export default function ImportInvoice1({ activeTab }) {
    ...
    const getSearchData = (val) => {
        ...
        axios.get(`${ipaddress}assessment/searchImportBeforeSaveAssessmentData?cid=${companyid}&bid=${branchId}&val=${val}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const getSelectedBeforeSearchData = (igmtrans, igm, line) => {
        setLoading(true);
        axios.get(`${ipaddress}assessment/getImportBeforeSaveAssessmentData?cid=${companyid}&bid=${branchId}&trans=${igmtrans}&igm=${igm}&lineNo=${line}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const handleSave = () => {
        ...
        axios.post(`${ipaddress}assessment/saveAssessmentData?cid=${companyid}&bid=${branchId}&user=${userId}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const saveProcess = () => {
        ...
        axios.post(`${ipaddress}assessment/saveImportInvoiceReceipt?cid=${companyid}&bid=${branchId}&user=${userId}&creditStatus=${assessmentData.creditType}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const searchExportEmptyContainerGateIn = (id) => {
        setLoading(true);
        axios.get(`${ipaddress}assessment/searchInvoiceData1?cid=${companyid}&bid=${branchId}&val=${id}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const getSelectedInvoiceData = (assId, invId) => {
        axios.get(`${ipaddress}assessment/getSelectedInvoiceData1?cid=${companyid}&bid=${branchId}&assId=${assId}&invId=${invId}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const getAllContainerDetailsOfAssesmentId = () => {
        try {
            ...
            axios.get(`${ipaddress}assessment/getDataByAfterAssessment?cid=${companyid}&bid=${branchId}&id=${assessmentData.assesmentId}&partyId=${partyId}&creditType=${assessmentData.creditType}`, {
                headers: {
                    Authorization: `Bearer ${jwtToken}`
                }
            })
    ...
    const saveAddServiceContainerWise = async (selectedContainers) => {
        ...
        try {
            const response = await FinanceService.addServiceExportInvoiceServiceWise1(companyid, branchId, userId, assessmentData, selectedContainers, jwtToken);
