...
export default function AuctionInvoice({ activeTab }) {
    ...
    const getSearchData = (val) => {
        ...
        axios.get(`${ipaddress}auctionInvoice/searchAuctionBeforeSaveAssessmentData?cid=${companyid}&bid=${branchId}&val=${val}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const handleSearchChange = async (selectedOption, { action }) => {
        ...
        if (action === 'clear') {
            ...
            getSelectedBeforeSearchData(selectedOption.igmTrans, selectedOption.value, selectedOption.lineNo, selectedOption.noticeId);
        }
    };


    const getSelectedBeforeSearchData = (igmtrans, igm, line, id) => {
        setLoading(true);
        axios.get(`${ipaddress}auctionInvoice/getAuctionBeforeSaveAssessmentData?cid=${companyid}&bid=${branchId}&trans=${igmtrans}&igm=${igm}&lineNo=${line}&id=${id}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
            .then((response) => {
                ...
                setAssessmentData({
                    ...
                    viaNo: singleData[7] || "",
                    igmDate: singleData[8] === null ? null : new Date(singleData[8]),
                    createdBy: "",
                    blNo: singleData[3] || "",
                    blDate: singleData[4] === null ? null : new Date(singleData[4]),
                    profitcentreId: singleData[5] || "",
                    sl: singleData[12] || "",
                    sa: singleData[10] || "",
                    insuranceValue: singleData[13] || "",
                    dutyValue: singleData[14] || "",
                    commodityDescription: singleData[15] || "",
                    commodityCode: "",
                    importerId: "",
                    importerName: singleData[19] || "",
                    impSrNo: "",
                    cha: "",
                    chaSrNo: "",
                    sez: "N",
                    ...
                    receiptNo: "",
                    intComments: "",
                    impAddress: (singleData[20] === null ? '' : singleData[20]) || '' + ' - ' + (singleData[21] === null ? '' : singleData[21]) || '' + ' - ' + (singleData[22] === null ? '' : singleData[22]) || '',
                    impGst: "",
                    chaAddress: "",
                    chaName: "",
                    chaGst: "",
                    ...
                    lastInvoiceDate: null,
                    sbNo: singleData[16] || "",
                    sbDate: singleData[17] === null ? null : new Date(singleData[17]),
                    noOf20ft: singleData[18] || ""
                })
                ...
                setContainerData(data.map((item) => ({
                    ...
                    assesmentLineNo: '',
                    containerNo: item[23] || '',
                    containerSize: item[24] || '',
                    containerType: item[25] || '',
                    gateInDate: item[26] === null ? null : new Date(item[26]),
                    destuffDate: null,
                    gateoutDate: null,
                    examPercentage: '',
                    typeOfContainer: '',
    ...
    const handleSave = () => {
        ...
        axios.post(`${ipaddress}auctionInvoice/saveAssessmentData?cid=${companyid}&bid=${branchId}&user=${userId}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
            .then((response) => {
                ...
                setAssessmentData({
                    ...
                    sbNo: singleData[58] || "",
                    sbDate: singleData[59] === null ? null : new Date(singleData[59]),
                    noOf20ft: singleData[60] || ""
                })
                ...
                setContainerData(data.map((item) => ({
                    ...
                    gateOutType: '',
                    checkDate: 'Y',
                    invoiceDate: item[80] === null ? null : new Date(item[80]),
                    lastInvoiceUptoDate: null,
                    upTariffNo: "",
                    profitcentreId: item[16] || "",
                    grossWt: "",
                    cargoWt: "",
                    ssrTransId: "",
                    serviceId: item[72] || "",
                    ...
                    bidAmt: item[68] || "0",
                    tcs: item[69] || "0",
                    igst: item[77] === 'Y' ? item[70] || '0' : '0',
                    cgst: item[78] === 'Y' ? (parseFloat(item[70])) / 2 || '0' : '0',
                    sgst: item[79] === 'Y' ? (parseFloat(item[70])) / 2 || '0' : '0',
                    rateOfDuty: item[71] || "0",
                })))
    ...
    const saveProcess = () => {
        ...
        axios.post(`${ipaddress}auctionInvoice/saveAuctionInvoiceReceipt?cid=${companyid}&bid=${branchId}&user=${userId}&creditStatus=${assessmentData.creditType}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const searchExportEmptyContainerGateIn = (id) => {
        setLoading(true);
        axios.get(`${ipaddress}auctionInvoice/searchInvoiceData1?cid=${companyid}&bid=${branchId}&val=${id}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const getSelectedInvoiceData = (assId, invId) => {
        axios.get(`${ipaddress}auctionInvoice/getSelectedInvoiceData1?cid=${companyid}&bid=${branchId}&assId=${assId}&invId=${invId}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const handlePrint1 = async (type) => {
        ...
        axios.post(`${ipaddress}importinvoiceprint/printauctioninvoicepdf/${companyid}/${branchId}/${invoiceNo}/${assesmentId}/${igmTransId}`, {
            headers: {
    ...
    const openAddServiceModal = async (item) => {
        ...
        setCfinvsrvanx(prevState => ({
            ...
            bidAmt: item.bidAmt || "",
            tcsRate: item.tcs || "0",
            taxPerc: (parseFloat(item.igst) + parseFloat(item.cgst) + parseFloat(item.sgst)) || 0,
            dutyRate: item.rateOfDuty || "0",
            prevRate: item.bidAmt
        }));
    ...
    const getAllContainerListOfAssessMentSheet = async (companyId, branchId, assesmentId, profitcentreId) => {
        try {
            const response = await FinanceService.getAuctionAllContainerListOfAssessMentSheet1(companyId, branchId, assesmentId, profitcentreId, jwtToken);
    ...
    const getAllContainerDetailsOfAssesmentId = () => {
        try {
            ...
            axios.get(`${ipaddress}auctionInvoice/getDataByAfterAssessment?cid=${companyid}&bid=${branchId}&id=${assessmentData.assesmentId}&partyId=${partyId}&creditType=${assessmentData.creditType}`, {
                headers: {
                    Authorization: `Bearer ${jwtToken}`
                }
            })
    ...
    const saveAddServiceContainerWise = async (selectedContainers) => {
        ...
        try {
            const response = await FinanceService.addServiceAuctionInvoiceServiceWise1(companyid, branchId, userId, assessmentData, selectedContainers, jwtToken);
            ...
            <div>
                <Row>
                    <Col md={4}>
                        <FormGroup>
                            <label className="forlabel bold-label" htmlFor="sbRequestId">
                                Search By IGM No / IGM Line No / BL No
                            </label>
                            <Select
                    ...
                    <Row>
                        ...
                        <Col md={2}>
                            <FormGroup>
                                ...
                                <div style={{ position: 'relative' }}>
                                    <DatePicker
                                        selected={assessmentData.sbDate}
                                        id='sbDate'
                                        name='sbDate'
                                        disabled
                                        dateFormat="dd/MM/yyyy HH:mm"
                        ...
                        <Col md={2}>
                            <FormGroup>
                                ...
                                <input
                                    ...
                                    id="noOf20ft"
                                    name='noOf20ft'
                                    value={assessmentData.noOf20ft === "P" ? "First" : assessmentData.noOf20ft === 'S' ? "Second" : assessmentData.noOf20ft === 'F' ? "Final" : ""}
                                    onChange={handleAssessmentChange}
                                    disabled
                                />
