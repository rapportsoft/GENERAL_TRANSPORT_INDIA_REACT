...
import { faIdBadge, faChartGantt, faBold, faBox, faArrowAltCircleLeft, faSearch, faRefresh, faUpload, faFileExcel, faSave, faCheck, faDownload, faTrash, faCalendarAlt, faAdd, faCancel, faXmark, faArrowDown, faPlus, faArrowUp, faEdit, faChevronUp, faChevronDown, faMagnifyingGlassChart, faProcedures, faSpinner, faPrint,  faFileAlt,
...
export default function GeneralPeriodiCommodityWiseInvoice({ activeTab }) {
    ...
    const getImpData = (val) => {
        ...
        axios.get(`${ipaddress}party/getImp1?cid=${companyid}&bid=${branchId}&val=${val}`, {
            headers: {
    ...
    const handleImpChange = async (selectedOption, { action }) => {
        ...
        if (action === 'clear') {
            ...
            setSearchImpId(selectedOption.value);
            setSearchImpName(selectedOption.label);
        }
    };

    const [impData1, setImpData1] = useState([]);

    const getImpData1 = (val) => {
        if (!val) {
            setImpData1([]);
            return;
        }
    ...
    const handleImpChange1 = async (selectedOption, { action }) => {
        ...
        if (action === 'clear') {
            ...
            if (assessmentData.importerId !== selectedOption.value) {
                handleClear();
            }
    ...
    const [searchImpId, setSearchImpId] = useState("");
    const [searchImpName, setSearchImpName] = useState("");
    ...
    const handleInvDate = (date) => {
        ...
        setContainerData((prevData) =>
            prevData.map((item) => {
                const gateInDate = item?.gateInDate ? new Date(item.gateInDate) : null;
                const lastInvoiceDate = item?.lastInvoiceUptoDate ? new Date(item.lastInvoiceUptoDate) : null;
                const invoiceDate = date ? new Date(date) : null;

                let baseDate = lastInvoiceDate || gateInDate;
                let weeks = '';

                // ✅ Only update if invoiceDate >= baseDate
                if (invoiceDate && baseDate && invoiceDate >= baseDate) {
                    const diffInMilliseconds = invoiceDate - baseDate;
                    const diffInDays = diffInMilliseconds / (1000 * 60 * 60 * 24);
                    weeks = Math.ceil(diffInDays / 7);
                ...
                // If date is less, return item as it is
                return item;
            })
        );
    ...
    const handleBeforeSaveSearch = () => {

        if (!searchImpId) {
            toast.error("Please select importer", {
                autoClose: 800
            })
            return;
        }
        ...
        axios.get(`${ipaddress}assessmentPeriodicCmdtyCon/searcBeforeSavePeriodicInvoice`, {
            ...
            .then((response) => {
                ...
                setAssessmentData({
                    ...
                    lastInvoiceDate: singleData[27] === null ? null : new Date(singleData[27]),
                ...
                setContainerData(data.map((item) => ({
                    ...
                    beNo: item[28] || "",
                    receivingId: item[16] || "",
                    lastInvoiceNo: item[26] || "",
                    lastInvoiceDate: item[27] === null ? null : new Date(item[27]),
                })));
            })
            .catch((error) => {
                setLoading(false);
    ...
    const handleSave = () => {
        try {
            ...
            if (!assessmentData.impGst) {
                toast.error("Importer GST is required!!", {
                    autoClose: 800
                })
                return;
            }
            ...
            axios.post(`${ipaddress}assessmentPeriodicCmdtyCon/saveContainerWiseGeneralPeriodicAssessmentData?cid=${companyid}&bid=${branchId}&user=${userId}`, formData, {
                headers: {
                    Authorization: `Bearer ${jwtToken}`
                }
            })
                .then((response) => {
                    ...
                    setAssessmentData({
                        ...
                        impAddress: singleData[19] ? singleData[19] + ' - ' + singleData[20] + ' - ' + singleData[21] : "",
                        impGst: singleData[22] || "",
                        chaAddress: singleData[32] ? singleData[32] + ' - ' + singleData[33] + ' - ' + singleData[34] : "",
                    ...
                    setContainerData(data.map((item) => ({
                        ...
                        beNo: item[80] || "",
                        receivingId: item[11] || "",
                        lastInvoiceNo: item[77] || "",
                        lastInvoiceDate: item[78] === null ? null : new Date(item[78]),
                    })))
    ...
    const openAddServiceModal = async (item) => {
        await getAllCfInvSrvAnxList(companyid, branchId, item.assesmentId, item.assesmentLineNo, item.containerNo, "N00008");
    ...
    const saveAddService = async (serviceData) => {
        ...
        setLoading(true);
        try {
            const response = await FinanceService.addServiceGeneralConInvoice(companyid, branchId, userId, assessmentData, serviceData, jwtToken, selectedInvoice);
    ...
    const openModelForAddServiceServiceWise = async () => {
        ...
        await getAllContainerListOfAssessMentSheet(companyid, branchId, assessmentData.assesmentId, "N00008");
    ...
    const getAllContainerDetailsOfAssesmentId = () => {
        try {
            ...
            axios.get(`${ipaddress}assessmentPeriodicCmdtyCon/getDataByAfterAssessmentForGeneralCon?cid=${companyid}&bid=${branchId}&id=${assessmentData.assesmentId}&partyId=${partyId}&creditType=${assessmentData.creditType}&type=${selectedInvoice}`, {
                headers: {
                    Authorization: `Bearer ${jwtToken}`
                }
            })
    ...
    const saveAddServiceContainerWise = async (selectedContainers) => {
        ...
        try {
            ...
            axios.post(`${ipaddress}assessmentPeriodicCmdtyCon/saveAddServiceForgeneralCon`, formData, {
                headers: {
                    Authorization: `Bearer ${jwtToken}`
                },
                params: {
    ...
    const saveProcess = () => {
        ...
        axios.post(`${ipaddress}assessmentPeriodicCmdtyCon/saveContainerWiseGeneralPeriodicInvoiceReceipt?cid=${companyid}&bid=${branchId}&user=${userId}&creditStatus=${assessmentData.creditType}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const searchExportEmptyContainerGateIn = (id) => {
        setLoading(true);
        axios.get(`${ipaddress}assessmentPeriodicCmdtyCon/searchPeriodicGeneralConInvoiceData?cid=${companyid}&bid=${branchId}&val=${id}&type=${selectedInvoice}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const getSelectedInvoiceData = (assId, invId) => {
        axios.get(`${ipaddress}assessmentPeriodicCmdtyCon/getSelectedGeneralConInvoiceData?cid=${companyid}&bid=${branchId}&assId=${assId}&invId=${invId}&type=${selectedInvoice}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const handlePrint1 = async (type) => {
        ...
        axios.post(`${ipaddress}importinvoiceprint/printContainerWiseGeneralPeriodicCommdtyInvoicepdfFor`, null, {
            headers: {
          ...
          <Alert color="warning" className="mt-3">
            ...
            <Row>
                ...
                <Col md={3}>
                    <FormGroup>
                        ...
                        <Select
                            value={{ value: searchImpId, label: searchImpName }}
                ...
                <Col md={3} style={{ marginTop: 22 }}>
                    <button
                        ...
                        id="submitbtn2"
                        onClick={handleBeforeSaveSearch}
                    >
                        <FontAwesomeIcon
                            icon={faSearch}
                ...
                <Col md={2}>
                    <FormGroup>
                        ...
                        <input
                            ...
                            id="status"
                            name='status'
                            value={assessmentData.status === "N" ? "New" : assessmentData.status === "A" ? "Approved" : assessmentData.status}
                            disabled
                        />
            ...
            <Row>
                ...
                <Col md={2}>
                    <FormGroup>
                        ...
                        <Select
                            ...
                            onChange={handleImpChange1}
                            onInputChange={getImpData1}
                            options={impData1}
                ...
                <Col md={2}>
                    <FormGroup>
                        <label className="forlabel bold-label" htmlFor="sbRequestId">
                            Importer GSTIN <span style={{ color: 'red' }}>*</span>
                        </label>
                        <input
                            className="form-control"
                            type="text"
