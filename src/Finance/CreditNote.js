...
function CreditNote({ activeTab }) {
    ...
    const handleClear = async (type) => {
        ...
        setInvoiceNoList([]);
        setSelectedRowsForCredit({});
        setCreditNoteHistory([]);
    }
...
// Add state to track which rows are selected
const [selectedRowsForCredit, setSelectedRowsForCredit] = useState({});

// Function to handle checkbox change for each row
// Function to handle checkbox change for each row
const handleInvoiceAmtCheckbox = (index, pendingAmt, item) => {
    // Prevent checkbox click if pending amount is <= 0
    if (pendingAmt <= 0) {
        toast.warning("Cannot select row with zero pending amount", {
            autoClose: 1000,
            position: 'top-center'
        });
        return;
    ...
    const isChecked = !selectedRowsForCredit[index];
    
    setSelectedRowsForCredit(prev => ({
        ...prev,
        [index]: isChecked
    }));
    
    // If checkbox is checked, set credit note amount to pending amount
    if (isChecked) {
        ...
        handleFieldChangeDetail(
            ...
            item.oldInvoiceAmt, 
            item.creditNoteAmt, 
            item.alreadySavedCreditAmt
        );
    } else {
        // If unchecked, set credit note amount to 0
        const syntheticEvent = {
            target: {
                value: '0'
            }
...
const handleSelectAllInvoice = () => {
    // Check if all rows with pendingAmt > 0 are already selected
    const selectableRows = creditNoteDtl.filter(item => parseFloat(item.pendingAmt) > 0);
    const selectedSelectableCount = creditNoteDtl.filter((item, index) => 
        selectedRowsForCredit[index] === true && parseFloat(item.pendingAmt) > 0
    ).length;
    
    const allSelectableSelected = selectableRows.length > 0 && selectedSelectableCount === selectableRows.length;
    
    if (!allSelectableSelected) {
        // Select only rows with pendingAmt > 0
        const newSelectedState = {};
        creditNoteDtl.forEach((item, index) => {
            const pendingAmt = parseFloat(item.pendingAmt) || 0;
            if (pendingAmt > 0) {
                newSelectedState[index] = true;
                const syntheticEvent = {
                    target: { value: pendingAmt.toString() }
                };
                handleFieldChangeDetail(
                    syntheticEvent, index, 'localAmt', 'decimal', 16, 3, 
                    pendingAmt, item.taxPerc, item.oldInvoiceAmt, 
                    item.creditNoteAmt, item.alreadySavedCreditAmt
                );
            }
        });
        setSelectedRowsForCredit(newSelectedState);
        toast.success(`${selectableRows.length} row(s) selected for credit note`, { autoClose: 1000 });
    } else {
        // Deselect all rows (clear all)
        creditNoteDtl.forEach((item, index) => {
            const syntheticEvent = {
                target: { value: '0' }
            };
            handleFieldChangeDetail(
                syntheticEvent, index, 'localAmt', 'decimal', 16, 3, 
                parseFloat(item.pendingAmt) || 0, item.taxPerc, 
                item.oldInvoiceAmt, item.creditNoteAmt, item.alreadySavedCreditAmt
            );
        });
        setSelectedRowsForCredit({});
        toast.info('All rows cleared', { autoClose: 1000 });
    }
};
    return (
        <>
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
        ...
        <input
            ...
            checked={creditNoteDtl.length > 0 && 
                    creditNoteDtl.filter(item => parseFloat(item.pendingAmt) > 0).length > 0 &&
                    Object.keys(selectedRowsForCredit).length === creditNoteDtl.filter(item => parseFloat(item.pendingAmt) > 0).length && 
                    Object.values(selectedRowsForCredit).every(v => v === true)}
            disabled={creditNoteHdr.invoiceNo?.startsWith("CJN")}
        ...
        />
                                            ...
                                            <tbody className='text-center'>
                                                ...
                                                {creditNoteDtl.map((item, index) => {
                                                  ...
                                                  const pendingAmt = parseFloat(item.pendingAmt) || 0;
                                                     const isChecked = selectedRowsForCredit[index] || false;
            ...
            return (
                                                    ...
                                                    <tr key={index}>
                                                        ...
                                                        {/* <td >
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
        {parseFloat(item.pendingAmt).toFixed(2)}
        {item.pendingAmt > 0 && (
            <input
                type="checkbox"
                checked={selectedRowsForCredit[index] || false}
                onChange={() => handleInvoiceAmtCheckbox(index, parseFloat(item.pendingAmt), item)}
                disabled={creditNoteHdr.invoiceNo?.startsWith("CJN")}
                style={{ transform: 'scale(1.3)', cursor: 'pointer' }}
            />
        )}
                                    ...
                                    if (isChecked && newValue !== pendingAmt) {
                                        setSelectedRowsForCredit(prev => ({
                                            ...prev,
                                            [index]: false
                                        }));
                                    }
                                    // Check checkbox if user manually enters pending amount
                                    if (!isChecked && newValue === pendingAmt && pendingAmt > 0) {
                                        setSelectedRowsForCredit(prev => ({
                                            ...prev,
                                            [index]: true
                                        }));
