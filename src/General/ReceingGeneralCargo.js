...
const dataToSave = validSelectedRows.map((row, idx) => {
  ...
  const calculateTotals = () => {
    ...
    selectedRows.forEach((row) => {
      const isInBondingIdValid =
        inBond?.receivingId != null && inBond?.receivingId !== "";
      const index = chaSearchedDatacargo.findIndex((item) =>
        inBond?.receivingId
          ? // If srNo exists, use commodityId
          item.jobTransId === row.jobTransId &&
          item.jobNo === row.jobNo &&
  ...
  const [deletedDrumSrNos, setDeletedDrumSrNos] = useState([]);
const deleteYardLocationDirect = (row, index) => {
  console.log("deleted");
  
  // Check if the row has a receivingId (already saved in database)
  if (row.receivingId) {
    setLoading(true);
    axios
      .delete(`${ipaddress}api/receiving/deleteYardLocationGridOnly`, {
        params: {
          companyId: companyid,
          branchId: branchId,
          receivingId: row.receivingId,
          ...
          yardBlock: row.yardBlock,
          blockCellNo: row.blockCellNo,
          cellAreaAllocated: row.cellAreaAllocated, // Pass the area to subtract
          user: userId,
        },
        headers: {
          Authorization: `Bearer ${jwtToken}`,
      ...
      .then((response) => {
        ...
        deleteRow(index);
        toast.success("Yard location deleted successfully!", {
          autoClose: 2000,
        });
        setLoading(false);
        // Optionally refresh grid data
        if (inBond.receivingId) {
          handleGridData(inBond.receivingId);
        }
      })
      .catch((error) => {
        setLoading(false);
        console.error("Error deleting yard location:", error);
        toast.error(
          error.response?.data || "Failed to delete yard location. Please try again.",
          {
            autoClose: 3000,
          }
        );
    ...
    // If not saved, just remove from UI
    deleteRow(index);
    toast.info("Row removed from UI.", {
      autoClose: 2000,
    });
  }
};
                  ...
                  <Col md={5}>
                    <FormGroup>
                      <label
                        ...
                        htmlFor="sbRequestId"
                      >
                        Search by Receiving Id / Importer /Job Trans Id/Job No/ BOE No
                      </label>
                      <input
                        className="form-control"
                        type="text"
...
<button
  className="dropdown-item"
  onClick={() => deleteYardLocationDirect(row, index)}
  disabled={row.deliveredPackages > 0 || row.invoiceNo}
  title={
    row.deliveredPackages > 0
      ? "Cannot delete row with delivered packages"
      : row.invoiceNo
      ? "Cannot delete row with invoice generated"
      : "Delete row"
  }
>
  <FontAwesomeIcon
