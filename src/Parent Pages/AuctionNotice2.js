...
function AuctionNotice2({ igm, igmLineNo, igmTrans, blNo, id1, id2, id3, acttab, listOfData, flag, onRequest, searchFlag }) {
...
const auctionDtlD = {
  ...
  const searchGate = (id) => {
    ...
    axios
      .get(
        `${ipaddress}api/auction/findAuctionDetailsSecond?companyId=${companyid}&branchId=${branchId}&partyName=${id}`,
        {
          headers: {
  ...
  useEffect(() => {
    if (acttab == "P01402") {
      if (id2 && igm && igmLineNo && igmTrans && searchFlag === 'Y') {
        selectGateSearchRadio(id2, "000", igm, igmLineNo, igmTrans);
      }
  ...
  }, [id2, igm, igmLineNo, igmTrans, acttab, searchFlag]);
...
shippingAgentName:data.sa,
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.post(
        `${ipaddress}api/auction/saveDataOfAuctionSecondNotice?companyId=${companyid}&branchId=${branchId}&user=${userId}&flag=${nocFlag}`,
        requestBody,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
  ...
  const handlePrint = async (type) => {
    setLoading(true);
        try {
            const response = await axios.get(`${ipaddress}api/auction/getPrintOfSecondNotice?companyId=${companyid}&branchId=${branchId}&uname=${username}&type=${type}&cname=${companyname}&bname=${branchname}&igmNo=${auctionDtl.igmNo}&igmLineNo=${auctionDtl.igmLineNo}&noticeId=${auctionDtl.noticeId}`,
            {
              headers:{
                Authorization: `Bearer ${jwtToken}`
              }
  ...
  const getIgmTransIdData = (val) => {
    ...
    axios
      .get(
        `${ipaddress}api/auction/getIgmTransIdSecondNotice?companyId=${companyid}&branchId=${branchId}&value=${val}`,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
          },
...
<Row>
  <Col md={2}>
    <FormGroup>
      ...
      <input className="form-control" type="text" id="noticeType" name="noticeType" value={auctionDtl.noticeType ==="S" ? "Second Notice":'Second Notice'} readOnly style={{ backgroundColor: "#E0E0E0" }} onChange={handlegateOutChange} />
    </FormGroup>
  </Col>
