...
export default function WaiverEntry({ process }) {
    ...
    const searchData = () => {
        ...
        setLoading(true);
        axios.get(`${ipaddress}waiver/checkIGMAndLineNo?cid=${companyid}&bid=${branchId}&igm=${igmNo}&lineNo=${itemNo}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
            .then((response) => {
                ...
                setIgmDtl({
                    igmNo: data.igmData[0] || "",
                    igmTransId: data.igmData[1] || "",
    ...
    const handleSave = () => {
        ...
        axios.post(`${ipaddress}waiver/saveData?cid=${companyid}&bid=${branchId}&igm=${igmDtl.igmNo}&igmTransId=${igmDtl.igmTransId}&lineNo=${igmDtl.igmLineNo}&user=${userId}`,
            formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`,
