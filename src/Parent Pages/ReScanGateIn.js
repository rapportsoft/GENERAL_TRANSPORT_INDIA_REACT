...
export default function ReScanGateIn({ igm, cont, conList, process, onRequest, inId, igmtrans }) {
    ...
    const saveData = () => {
        ...
        if (!gateIn.inVehicleNo) {
            errors.inVehicleNo = "In Vehicle No is required."
            document.getElementById("inVehicleNo").classList.add('error-border');
        }

        if (!gateIn.inTransporterName) {
            errors.inTransporterName = "In Transporter name is required."
            document.getElementById("inTransporterName").classList.add('error-border');
        }
        ...
        axios.post(`${ipaddress}rescangateout/saveReScanGateInData?cid=${companyid}&bid=${branchId}&user=${userId}&flag=${flag}`, gateIn, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
        ...
        axios.get(`${ipaddress}gateIn/getSingleData?cid=${companyid}&bid=${branchId}&gateinid=${gatein}&igmtrans=${igmtrans}&igm=${igm}`, {
            ...
            .then((response) => {
                ...
                setGateIn(prevState => ({
                    ...
                    lowBed: data.lowBed || 'N',
                    createdDate: data.createdDate || null,
                    inVehicleNo: data.inVehicleNo || '',
                    inTransporterStatus: data.inTransporterStatus || '',
                    inTransporter: data.inTransporter || '',
                    inTransporterName: data.inTransporterName || '',
                }));
                setProfit(profit);
                setVessel(vessel);
                setPn(pn);
    ...
    const getContainersForSearch = (val) => {
        ...
        axios.get(`${ipaddress}rescangateout/searchBeforeSaveDataForRescangatein?cid=${companyid}&bid=${branchId}&val=${val}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const handleConChange = async (selectedOption, { action }) => {
        ...
        if (action === 'clear') {
            ...
            getSingleGateInDataBeforeSave(selectedOption.id);
        }
    ...
    useEffect(() => {
        if (process === 'P00218' && !gateIn.gateInId) {
    ...
    const getSingleGateInDataBeforeSave = (gatein) => {
        if (!gatein) {
            return;
        }

        axios.get(`${ipaddress}rescangateout/getSelectSearchBeforeSaveDataForRescangateout?cid=${companyid}&bid=${branchId}&val=${gatein}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
            .then((response) => {
                const data = response.data.gateIn;
                setFlag('add');
                ...
                setGateIn(prevState => ({
                    ...
                    yardBlock1: data.yardBlock1 || prevState.yardBlock1,
                    yardCell1: data.yardCell1 || prevState.yardCell1,
                    transporterStatus: data.outTransporterStatus || 'C',
                    transporterName: data.outTransporterName || prevState.transporterName,
                    vehicleNo: data.outVehicleNo || prevState.vehicleNo,
                    driverName: '',
                    comments: '',
                    specialRemarks: data.specialRemarks || prevState.specialRemarks,
                    status: '',
                ...
                const data1 = response.data.gateIn1;
            ...
            <div>
                ...
                <Row>
                    ...
                    <Col md={2}>
                        <FormGroup>
                            <label className="forlabel bold-label" htmlFor="sbRequestId">
                                In Vehicle No <span style={{ color: 'red' }}>*</span>
                            </label>
                            <Input
                                className="form-control"
                                type="text"
                                id="inVehicleNo"
                                name='inVehicleNo'
                                value={gateIn.inVehicleNo}
                                onChange={handleInputChange}
                            />
                            <div style={{ color: 'red' }} className="error-message">{formErrors.inVehicleNo}</div>
                        </FormGroup>
                    </Col>
                    <Col md={2}>
                        <FormGroup>
                ...
                <Row>
                    ...
                    <Col md={2}>
                        <FormGroup>
                            <label className="forlabel bold-label" htmlFor="sbRequestId">
                                In Transporter Name <span style={{ color: 'red' }}>*</span>
                            </label>
                            <Input
                                className="form-control"
                                type="text"
                                maxLength={50}
                                id="inTransporterName"
                                name='inTransporterName'
                                value={gateIn.inTransporterName}
                                onChange={handleInputChange}
                            />
                            <div style={{ color: 'red' }} className="error-message">{formErrors.inTransporterName}</div>
                        </FormGroup>
                    </Col>
                    <Col md={2}>
                        <FormGroup>
                            <label className="forlabel bold-label" htmlFor="sbRequestId">
                                In Transporter Status
                            </label>
                            <Input
                                className="form-control"
                                type="select"
                                id="inTransporterStatus"
                                name='inTransporterStatus'
                                value={gateIn.inTransporterStatus}
                                onChange={handleInputChange}
                            >
