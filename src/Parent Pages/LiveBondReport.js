...
export default function LiveBondReport() {
    ...
    const [section49Flag ,setSection49Flag] =useState('N');
    const handleStartDateChange = (date) => {
      ...
      const handleDownload =()=>{
        ...
        console.log("section49Flag ",section49Flag);
      ...
      const handleExleDownload = async (type) => {
        try {
          ...
          const response = await axios.get(
            `${ipaddress}api/bondingReport/liveBondReport?companyId=${companyid}&branchId=${branchId}&uname=${username}&type=${type}&cname=${companyname}&bname=${branchname}&startDate=${formattedStartDate}&endDate=${formattedEndDate}&section49=${section49Flag}`,
            // `${ipaddress}api/bondingReport/downLoadmontlyReport?companyId=${companyid}&branchId=${branchId}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`,
            {
              headers: {
          ...
          a.href = url;
          a.download = `LiveBondReport.xlsx`;
             ...
             <div>
                      <DatePicker
                       ...
                       name='startDate'
                        ...
                        dateFormat="dd/MM/yyyy" // 24-hour format for date and time
                        showTimeInput
...
<div>
      ...
      <Col md={2}>
  <FormGroup>
    <label className="forlabel bold-label" htmlFor="section49Flag">
      Section 49
    </label>
    <div className="form-check">
      <input
        ...
        type="checkbox"
        id="section49Flag"
        checked={section49Flag === 'Y'} // Checked if section49Flag is 'Y'
        onChange={(e) => setSection49Flag(e.target.checked ? 'Y' : 'N')} // Set 'Y' if checked, otherwise 'N'
      />
    </div>
  </FormGroup>
</Col>
