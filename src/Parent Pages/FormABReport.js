...
export default function FormABReport() {
    ...
    setInventoryList(data);
      ...
      //   }
      //     const response = await axios.get(
      //       `${ipaddress}api/bondingReport/downLoadformAB?companyId=${companyid}&branchId=${branchId}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`,
      //       {
      //         headers: {
      //           Authorization: `Bearer ${jwtToken}`
      //         },
      ...
      const DownloadAB = async () => {
        setLoading(true);
        try {
            // const response = await InBondService.downLoadDataForFormAandFormb({ params: searchCriteria });
    
                  const formattedStartDate = startDate ? moment(startDate).format('YYYY-MM-DD HH:mm') : '';
          const formattedEndDate = endDate ? moment(endDate).format('YYYY-MM-DD HH:mm') : '';
                      const response = await axios.get(
            `${ipaddress}api/bondingReport/downLoadformAB?companyId=${companyid}&branchId=${branchId}&startDate=${formattedStartDate}&endDate=${formattedEndDate}&cname=${companyname}&bname=${branchname}`,
            {
              headers: {
                Authorization: `Bearer ${jwtToken}`
              },
            ...
            const fileUrl = response.data; // Assuming response.data contains the base URL
    
            const link = document.createElement('a');
            link.href = fileUrl;
            link.setAttribute('download', 'FormA/BReport.xls'); // Ensure the filename has the .xls extension
