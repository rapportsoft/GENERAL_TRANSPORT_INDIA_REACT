...
export default function NocDepositeReport() {
    ...
    const handleStartDateChange = (date) => {
      ...
      const handleExleDownload = async (type) => {
        try {
          ...
          a.href = url;
          a.download = `NocDepositeRegister_${exBondingId}.xlsx`;
      ...
      const handleShow = async (type) => {
        try {
          if (! endDate && !startDate)
          {
            ...
            const response = await axios.get(
                `${ipaddress}api/bondingReport/showNocRegister?companyId=${companyid}&branchId=${branchId}&uname=${username}&type=${type}&cname=${companyname}&bname=${branchname}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`,
                {
                    headers: {
                        Authorization: `Bearer ${jwtToken}`
                    }
...
<div>
    ...
    <th scope="col" style={{minWidth:180}}>Date</th>
    <th scope="col">NOC No.</th>
    <th scope="col" style={{minWidth:360}}>Importer Name</th>
    <th scope="col" style={{minWidth:360}}>CHA Name</th>
    <th scope="col">B/E No.</th>
    <th scope="col" style={{minWidth:180}}>B/E Date</th>
    <th scope="col">NOC Package</th>
    <th scope="col">B/E Weight</th>
    <th scope="col">Area</th>
    <th scope="col">Cargo Value</th>
    <th scope="col">Duty</th>
    <th scope="col" style={{minWidth:180}}>NOC Validity</th>
    <th scope="col" style={{minWidth:180}}>Inbond Date</th>
    ...
    {currentItems.map((item, index) => (
     ...
     <td>{item[13] || ""}</td> {/* Cargo Description */}
   </tr>
