...
export default function ReScanGateOut({ igm, cont, conList, process, onRequest, inId, igmtrans }) {
    ...
    const saveData = () => {
        ...
        if (!gateIn.outVehicleNo) {
            errors.outVehicleNo = "Out Vehicle No is required."
            document.getElementById("outVehicleNo").classList.add('error-border');
        }

        if (!gateIn.outTransporterName) {
            errors.outTransporterName = "Out Transporter name is required."
            document.getElementById("outTransporterName").classList.add('error-border');
        }
        ...
        axios.post(`${ipaddress}rescangateout/saveGateIn?cid=${companyid}&bid=${branchId}&user=${userId}&flag=${flag}`, gateIn, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
        ...
        axios.get(`${ipaddress}gateIn/getSingleData?cid=${companyid}&bid=${branchId}&gateinid=${gatein}&igmtrans=${igmtrans}&igm=${igm}`, {
            ...
            .then((response) => {
                ...
                setGateIn(prevState => ({
                    ...
                    lowBed: data.lowBed || 'N',
                    createdDate: data.createdDate || null,
                    outVehicleNo: data.outVehicleNo || '',
                    outTransporterStatus: data.outTransporterStatus || '',
                    outTransporter: data.outTransporter || '',
                    outTransporterName: data.outTransporterName || '',
                }));
                setProfit(profit);
                setVessel(vessel);
                setPn(pn);
    ...
    const getContainersForSearch = (val) => {
        ...
        axios.get(`${ipaddress}rescangateout/searchBeforeSaveDataForRescangateout?cid=${companyid}&bid=${branchId}&val=${val}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    useEffect(() => {
        if (process === 'P00217' && !gateIn.gateInId) {
            ...
            <div>
                ...
                <Row>
                    ...
                    <Col md={2}>
                        <FormGroup>
                            <label className="forlabel bold-label" htmlFor="sbRequestId">
                                Out Vehicle No <span style={{ color: 'red' }}>*</span>
                            </label>
                            <Input
                                className="form-control"
                                type="text"
                                id="outVehicleNo"
                                name='outVehicleNo'
                                value={gateIn.outVehicleNo}
                                onChange={handleInputChange}
                            />
                            <div style={{ color: 'red' }} className="error-message">{formErrors.outVehicleNo}</div>
                        </FormGroup>
                    </Col>
                    <Col md={2}>
                        <FormGroup>
                ...
                <Row>
                    ...
                    <Col md={2}>
                        <FormGroup>
                            <label className="forlabel bold-label" htmlFor="sbRequestId">
                                Out Transporter Name <span style={{ color: 'red' }}>*</span>
                            </label>
                            <Input
                                className="form-control"
                                type="text"
                                maxLength={50}
                                id="outTransporterName"
                                name='outTransporterName'
                                value={gateIn.outTransporterName}
                                onChange={handleInputChange}
                            />
                            <div style={{ color: 'red' }} className="error-message">{formErrors.outTransporterName}</div>
                        </FormGroup>
                    </Col>
                    <Col md={2}>
                        <FormGroup>
                            <label className="forlabel bold-label" htmlFor="sbRequestId">
                                Out Transporter Status
                            </label>
                            <Input
                                className="form-control"
                                type="select"
                                id="outTransporterStatus"
                                name='outTransporterStatus'
                                value={gateIn.outTransporterStatus}
                                onChange={handleInputChange}
                            >
