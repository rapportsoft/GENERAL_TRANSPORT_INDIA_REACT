...
export default function ExportWaiver({ searchData, resetFlag, updatePagesList }) {
    ...
    useEffect(() => {
        if (searchData.activeTab === 'P00248') {
            getServices();
        }
    }, [searchData])
    ...
    const searchData1 = () => {
        ...
        setLoading(true);
        axios.get(`${ipaddress}waiver/checkSBNo?cid=${companyid}&bid=${branchId}&sbNo=${igmNo}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
            .then((response) => {
                ...
                setIgmDtl({
                    igmNo: data.igmData[1] || "",
                    igmTransId: data.igmData[0] || "",
                    igmLineNo: data.igmData[2] || ""
                })
    ...
    const handleSave = () => {
        ...
        axios.post(`${ipaddress}waiver/saveSBNOData?cid=${companyid}&bid=${branchId}&igm=${igmDtl.igmNo}&igmTransId=${igmDtl.igmTransId}&lineNo=${igmDtl.igmLineNo}&user=${userId}`,
            formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`,
