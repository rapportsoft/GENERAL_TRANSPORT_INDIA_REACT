...
export default function Section49ExpiredBondReport() {
      ...
      const handleShow = async (type) => {
        try {
            ...
            const response = await axios.get(
                `${ipaddress}api/bondingReport/showBondSectionExpiredBondReport?companyId=${companyid}&branchId=${branchId}&uname=${username}&type=${type}&cname=${companyname}&bname=${branchname}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`,
                {
                    headers: {
    ...
    const handleClear =()=>{
      ...
      setEndDate(new Date());
        ...
        try {
          ...
          const response = await axios.get(
            `${ipaddress}api/bondingReport/sectionExpiredBondReport?companyId=${companyid}&branchId=${branchId}&uname=${username}&type=${type}&cname=${companyname}&bname=${branchname}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`,
            {
              headers: {
                Authorization: `Bearer ${jwtToken}`
              },
          ...
          a.href = url;
          a.download = "Section49ExpiredBondReport.xlsx";
...
<div>
    ...
    <thead className="text-center">
        <tr>
            ...
            <th scope="col">Section 49(Yes/No)</th>
    ...
    <tbody className="text-center">
        {currentItems.map((item, index) => (
            <tr key={index}>
                ...
                <td>
                    <input style={{ width: '100px', border: 'none', outline: 'none', boxShadow: 'none' }} value={item.section49 =="Y" ? "YES" :item.section49 =="N" ? "NO" :'' } />
                </td>
                <td>
                ...
                <td>
                    <input style={{ width: '54px', border: 'none', outline: 'none', boxShadow: 'none' }} value={item.igmNo || ""} />
                </td>
                <td>
                    <input style={{ width: '54px', border: 'none', outline: 'none', boxShadow: 'none' }} value={item.igmLineNo || ""} />
                </td>
                <td>
