...
export default function SSRActivity({ process }) {
    ...
    const handleSearch = () => {
        ...
        axios.get(`${ipaddress}ssr/search?cid=${companyid}&bid=${branchId}&igm=${searchIgm}&item=${searchItem}&container=${searchContainer}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
            .then((response) => {
                ...
                setSsrDtl({
                    ...
                    branchId: "",
                    transId: "",
                    erpDocRefNo: crgData.igmTransId || "",
                    docRefNo: crgData.igmNo || "",
                    containerNo: "",
                    profitcentreId: crgData.profitcentreId || "",
                    serviceId: "",
                    transLineNo: null,   // BigDecimal
                    transDate: new Date() || null,     // Date (You can use null for dates)
                    ssrRefNo: "",
                    docRefDate: new Date(crgData.igmDate) || null,    // Date
                    igmLineNo: crgData.igmLineNo || "",
                    blNo: crgData.blNo || "",
                    blDate: new Date(crgData.blDate) || null,        // Date
                    beNo: crgData.beNo || "",
                    beDate: crgData.beDate != null ? new Date(crgData.beDate) : null,        // Date
                    containerSize: "",
                    containerType: "",
                    noOfPackages: 0,     // int
                    cargoWt: null,       // BigDecimal
                    gateOutType: "",
                    containerInvoiceType: "",
                    commodityDescription: crgData.commodityDescription || "",
                    impId: crgData.importerName || "",
                    sl: crgData.sl || "",
                    sa: crgData.sa || "",
                    cha: crgData.chaName || "",
                    accId: crgData.accountHolderName || "",
                    ssrModeFor: "",
                    srlNo: null,         // BigDecimal
                    serviceUnit: "",
                    executionUnit: "",
    ...
    const handleSave = () => {
        ...
        axios.post(`${ipaddress}ssr/saveData?cid=${companyid}&bid=${branchId}&user=${userId}`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const search = (val) => {
        ...
        axios.get(`${ipaddress}ssr/searchSSR?cid=${companyid}&bid=${branchId}&val=${val}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const getDataByTransId = (trans, igmtrans, igmno, line) => {
        setLoading(true);
        axios.get(`${ipaddress}ssr/getDataByTransId?cid=${companyid}&bid=${branchId}&transid=${trans}&igmtrans=${igmtrans}&igmno=${igmno}&line=${line}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
        ...
        <div>
                            ...
                            <table className="table table-bordered table-hover tableHeader">
                                ...
                                <tbody>
                                    {currentItems.map((item, index) => (
                                        <tr key={index}>
                                            <td>
                                                <input type="radio" onChange={()=>getDataByTransId(item[0],item[1],item[2],item[3])} name="radioGroup" />
                                            </td>
                                            <td>{item[0]}</td>
                                            <td>{item[1]}</td>
                                            <td>{item[2]}</td>
                ...
                <Col md={2}>
                    <FormGroup>
                        ...
                        <Input
                            ...
                            name='searchIgm'
                            value={searchIgm}
                            disabled={searchContainer !== '' || ssrDtl.transId !== ''}
            ...
            <Row>
                ...
                <Col md={2}>
                    <FormGroup>
                        <label className="forlabel bold-label" htmlFor="sbRequestId">
                            IGM No <span style={{ color: 'red' }}>*</span>
                        </label>
                        <Input
                            className={`form-control`}
                            type="text"
                ...
                <table className="table table-bordered table-hover tableHeader dynamic-table">
                    <thead className="tableHeader">
                        ...
                        <tr>
                            ...
                            <th scope="col" className="text-center" style={{ color: 'black' }}>Cargo Wt</th>
                            <th scope="col" className="text-center" style={{ color: 'black' }}>Delivery Type</th>
