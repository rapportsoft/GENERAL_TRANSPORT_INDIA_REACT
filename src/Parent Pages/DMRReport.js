import {
  ...
  export default function DMRReport() {
    ...
    const handlePortChange = async (selectedOption, { action }) => {
      if (action === "clear") {
        ...
        setProfitCenter(selectedOption ? selectedOption.value : "");
    ...
    const getIsoData = (val) => {
      ...
      axios
        .get(
          `${ipaddress}api/profitcentres/getProfitcenterAllData?companyId=${companyid}&branchId=${branchId}&desc=${val}`,
          {
            headers: {
    ...
    const handleAccountHolderChange = async (selectedOption, { action }) => {
      if (action === "clear") {
        ...
        setAccountHolderName(selectedOption ? selectedOption.label : "");
      }
    };
    ...
    const getAccountHolderData = (val) => {
      ...
      axios
        .get(
          `${ipaddress}api/dmrReport/searchAccountHolder?companyId=${companyid}&branchId=${branchId}&lin=${shippingLineSelected}&agt=${shippingAgentSelected}&cha=${chaSelected}&partyName=${val}`,
          {
            headers: {
              Authorization: `Bearer ${jwtToken}`,
            },
    ...
    const handleExleDownload = async (type) => {
      try {
        ...
        const formattedEndDate = endDate
          ...
          : "";
                  if (!shippingAgentSelected && !shippingLineSelected && !accountHolderSelected && !chaSelected) {
            toast.error("Please select at least one party.");
            setLoading(false); // Ensure the loading state is reset
            return;
          }
           if(!accountHolder || !startDate || !profitcenter)
           {
            toast.error("Please select all mandetory fields.");
            return;
           }
        ...
        const response = await axios.get(
          `${ipaddress}api/dmrReport/dmrReportProfitCenterWise?companyId=${companyid}&branchId=${branchId}&uname=${username}&type=${type}&cname=${companyname}&bname=${branchname}&startDate=${formattedStartDate}&endDate=${formattedEndDate}&accountHolderName=${accountHolderName}&accountHolder=${accountHolder}&profitcenter=${profitcenter}`,
          {
            headers: {
              Authorization: `Bearer ${jwtToken}`,
            },
        ...
        a.href = url;
        a.download = `${accountHolderName}.xlsx`;
    ...
    const [accountHolderSelected, setAccountHolderSelected] = useState("");
    const [shippingLineSelected, setShippingLineSelected] = useState("");
    const [shippingAgentSelected, setShippingAgentSelected] = useState("");
    const [chaSelected, setChaSelected] = useState("");
    const [importerSelected, setImporterSelected] = useState("");
  
    const handleAccountHolderRadioChange = () => {
      setAccountHolderSelected("Y");
      setShippingLineSelected("");
      setShippingAgentSelected("");
      setAccountHolderData([]);
      setAccountHolderName('');
      setAccountHolder('');
      setChaSelected('');
      console.log("Selected Report: Y (Account Holder)");
    };
    const handleChaHolderRadioChange = () => {
      setChaSelected("Y");
      setAccountHolderSelected("");
      setShippingLineSelected("");
      setShippingAgentSelected("");
      setAccountHolderData([]);
      setAccountHolderName('');
      setAccountHolder('');
      console.log("Selected Report: Y (handleChaHolderRadioChange Cha)");
    };
  
    // Handle Shipping Line selection
    const handleShippingLineChange = () => {
      setShippingLineSelected("Y");
      setAccountHolderSelected("");
      setShippingAgentSelected("");
      setAccountHolderName('');
      setAccountHolder('');
      setAccountHolderData([]);
      setChaSelected('');
      console.log("Selected Report: Y (Shipping Line)");
    };
  
    // Handle Shipping Agent selection
    const handleShippingAgentChange = () => {
      setShippingAgentSelected("Y"); // Set value to Y when Shipping Agent is selected
      setShippingLineSelected("");
      setAccountHolderSelected("");
      setAccountHolderData([]);
      setAccountHolderName('');
      setChaSelected('');
      setAccountHolder('');
      console.log("Selected Report: Y (Shipping Agent)");
    };
            ...
            <Col md={4}>
              <FormGroup>
                <label className="forlabel bold-label" htmlFor="accountHolder">
                  Account Holder <span className="error-message">*</span>
                </label>
                <Select
            ...
            <Col md={4}>
              <FormGroup>
                <label className="forlabel bold-label" htmlFor="sbRequestId">
                  Profit Center <span className="error-message">*</span>
                </label>
                <Select
                  value={{ value: profitcenter, label: chaName }}
                ...
                {/* <div style={{ color: 'red' }} className="error-message">{nocErrors.profitcenter}</div> */}
              </FormGroup>
            </Col>
          </Row>
          <Row></Row>
              ...
              <Col md={3} className="mb-2">
                <div className="d-flex align-items-center">
                  <input
                    ...
                    id="radio-1"
                    name="reportGroup"
                    checked={accountHolderSelected === "Y"}
                    onChange={handleAccountHolderRadioChange} // Handle change for Account Holder
                  />
                  <label className="ms-2 bold-report" htmlFor="radio-1">
                    Account Holder
                  </label>
                </div>
              </Col>
  
              {/* Shipping Line Radio Button */}
              <Col md={3} className="mb-2">
                <div className="d-flex align-items-center">
                  <input
                    type="radio"
                    id="radio-2"
                    name="reportGroup"
                    checked={shippingLineSelected === "Y"}
                    onChange={handleShippingLineChange} // Handle change for Shipping Line
                  />
                  <label className="ms-2 bold-report" htmlFor="radio-2">
                    Shipping Line
                  </label>
                </div>
              </Col>
  
              {/* Shipping Agent Radio Button */}
              <Col md={3} className="mb-2">
                <div className="d-flex align-items-center">
                  <input
                    type="radio"
                    id="radio-3"
                    name="reportGroup"
                    checked={shippingAgentSelected === "Y"}
                    onChange={handleShippingAgentChange} // Handle change for Shipping Agent
                  />
                  <label className="ms-2 bold-report" htmlFor="radio-3">
                    Shipping Agent
                  </label>
                </div>
              </Col>
              <Col md={3} className="mb-2">
                <div className="d-flex align-items-center">
                  <input
                    ...
                    id="radio-1"
                    name="reportGroup"
                    checked={chaSelected === "Y"}
                    onChange={handleChaHolderRadioChange} // Handle change for Account Holder
                  />
                  <label className="ms-2 bold-report" htmlFor="radio-1">
                    Cha
                  </label>
