...
import { faRefresh, faMoneyBill, faCalendarAlt, faEdit, faSave, faUpload, faDownload, faFileInvoiceDollar, faHistory, faCheck } from '@fortawesome/free-solid-svg-icons';
...
function Quotation() {
    ...
    const getAllActiveServices = async () => {
        try {
            const response = await FinanceService.getAllServicesQuatation(companyid, branchId, jwtToken);
    ...
    const searchSavedTariff = async (type) => {
        try {
            const response = await FinanceService.searchSavedTariffQuatation(companyid, branchId, type, jwtToken);
    ...
    const searchTarrifMain = async (searchCriteria) => {
        try {
            const response = await FinanceService.getSavedTariffQuatation(searchCriteria, jwtToken);
    ...
    const searchExporter = async (searchValue, type) => {
        ...
        try {
            const response = await FinanceService.searchExporterQuatation(companyid, branchId, searchValue, type, jwtToken);
    ...
    const handleServiceClick = async (service) => {
        ...
        try {
            const response = await FinanceService.getSingleServiceQuatation(companyid, branchId, service.serviceId, cfsTariff.cfsTariffNo, cfsTariff.cfsAmndNo, jwtToken);
            setService(service);
    ...
    const handleSaveMainTariff = async (type) => {
        ...
        setLoading(true);
        try {
            const response = await FinanceService.addHeaderCfsTariffQuatation(companyid, branchId, userId, cfsTariff, type, jwtToken);
            setCfsTarrif(response.data);
            let successMessege;
            ...
            if (error.response && error.response.status === 400) {
                ...
                if (typeof errorMessage === 'object' && errorMessage.details) {
                    ...
                    }else if (field === 'contractNameHeader') {
                        // Handle Duplicate Container No
                        const { contractName } = details;

                        const errorMessageNew = `Duplicate ContractName In main tariff: <strong>${contractName}</strong>`;
                    ...
                    } else if (field === 'combinationHeader') {

                        const errorMessageNew = `Selected combination already exist In main tariff`;
                        toast.error(<div dangerouslySetInnerHTML={{ __html: errorMessageNew }} />, {
                            position: 'top-center',
                            autoClose: 3000,
                            style : {width: '27vw'}
    ...
    const handleSaveDetailTariff = async (cfsTariffService) => {
        ...
        setLoading(true);
        try {
            const response = await FinanceService.addDetailCfsTariffServiceQuatation(companyid, branchId, cfsTariff, checkedDetails, userId, jwtToken);
    ...
    const handleAmmend = async () => {
        ...
        try {
            const response = await FinanceService.ammendTariffQuatation(companyid, branchId, userId, cfsTariff, jwtToken);
    ...
    const downLoadReport = async (type) => {
        ...
        try {
            const response = await FinanceService.downLoadTariffReportQuatation(dataTosend, jwtToken);
    ...
    const handleUploadTariff = async () => {
        setLoading(true);
        try {
            const response = await FinanceService.uploadTariffQuatation(companyid, branchId, userId, cfsTariff, jwtToken);
            setCfsTarrif(response.data);
    ...
    const handleDownLoadTemplate = async () => {
        ...
        setLoading(true);
        try {
            const response = await FinanceService.downLoadTariffTemplateQuatation(dataTosend, jwtToken);
