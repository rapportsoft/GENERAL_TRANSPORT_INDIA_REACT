...
function AuctionNoticeFinal({ igm, igmLineNo, igmTrans, blNo, id1, id2, id3, acttab, listOfData, flag, onRequest, searchFlag }) {
...
const auctionDtlD = {
  ...
  const searchGate = (id) => {
    ...
    axios
      .get(
        `${ipaddress}api/auction/findAuctionDetailsFinal?companyId=${companyid}&branchId=${branchId}&partyName=${id}`,
        {
          headers: {
  ...
  useEffect(() => {
    if (acttab == "P01403") {
      if (id3 && igm && igmLineNo && igmTrans && searchFlag === 'Y') {
        selectGateSearchRadio(id3, "000", igm, igmLineNo, igmTrans);
      }
  ...
  }, [id3, igm, igmLineNo, igmTrans, acttab, searchFlag]);
  ...
  const selectGateSearchRadio = (
    ...
    axios
      ...
      .then((response) => {
        ...
        console.log("getDataByPartyIdAndGstNoYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY", firstItem);
        setAuctionDtl(firstItem);

        setAuctionDtl((pre)=>({
...pre,
shippingAgentName:firstItem.sa,
        }));
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.post(
        `${ipaddress}api/auction/saveDataOfAuctionFinalNotice?companyId=${companyid}&branchId=${branchId}&user=${userId}&flag=${nocFlag}`,
        requestBody,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
      ...
      console.log("Data saved successfully!! Response", response.firstItem);
      fetchDataAfterSave(companyid,branchId,firstItem.noticeId,firstItem.noticeAmndNo);
  ...
  const handlePrint = async (type) => {
    setLoading(true);
        try {
            const response = await axios.get(`${ipaddress}api/auction/getPrintOfFinalNotice?companyId=${companyid}&branchId=${branchId}&uname=${username}&type=${type}&cname=${companyname}&bname=${branchname}&igmNo=${auctionDtl.igmNo}&igmLineNo=${auctionDtl.igmLineNo}&noticeId=${auctionDtl.noticeId}`,
            {
              headers:{
                Authorization: `Bearer ${jwtToken}`
              }
  ...
  const getIgmTransIdData = (val) => {
    ...
    axios
      .get(
        `${ipaddress}api/auction/getIgmTransIdFinalNotice?companyId=${companyid}&branchId=${branchId}&value=${val}`,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
          },
...
<Row>
  <Col md={2}>
    <FormGroup>
      ...
      <input className="form-control" type="text" id="noticeType" name="noticeType" value={auctionDtl.noticeType ==="F" ? 'Final Notice':'Final Notice'} readOnly style={{ backgroundColor: "#E0E0E0" }} onChange={handlegateOutChange} />
    </FormGroup>
  </Col>
...
export default AuctionNoticeFinal;
