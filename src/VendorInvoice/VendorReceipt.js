...
export default function VendorReceipt() {
    ...
    const getAccData = (val) => {
        ...
        axios.get(`${ipaddress}vendorInvoice/getAllWithAdd?cid=${companyid}&bid=${branchId}&val=${val}`, {
            headers: {
    ...
    const handleAccChange = async (selectedOption, { action }) => {
        ...
        if (action === 'clear') {
            ...
            setFinTrans((prev) => ({
                ...
                accountName: selectedOption.accName,
                partyBankName: selectedOption.bankName,
                bankAddress: selectedOption.bankAddress,
                ifscCode: selectedOption.ifscCode,
                accountNo: selectedOption.accountNo
            }));
    ...
    const getAdvanceData = (id) => {
        axios.get(`${ipaddress}vendorInvoice/advanceReceiptBeforeSaveSearch`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            },
            params: {
                ...
                if (finTrans.docType === "RE") {
                    ...
                    setInvoiceData(data.map((item) => {
                        ...
                        return {
                            ...
                            tdsAmt: tdsAmt.toFixed(2), // Format to 2 decimal places
                            localAmount: item[23] || '',
                            transId: item[14] || '',
    ...
    const handleSave1 = () => {
        ...
        for (let i = 0; i < paymentMode.length; i++) {
            ...
            if ((!chequeNo || !chequeDate) && payMode !== 'CASH' && payMode !== 'TDS' && payMode !== 'ADVANCE') {
                toast.error(`Error: Transaction details are missing for payment mode entry ${i + 1}.`, {
                    autoClose: 800,
                });
                return; // Stop the process if validation fails
            }
        ...
        axios.post(`${ipaddress}vendorInvoice/saveReceipt`, formData, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            },
            params: {
            ...
            .then((response) => {
                ...
                setFinTrans({
                    ...
                    accountName: singleData[19] || "",
                    partyBankName: singleData[20] || "",
                    bankAddress: singleData[21] || "",
                    ifscCode: singleData[22] || "",
                    accountNo: singleData[23] || "",
                })
    ...
    const searchExportEmptyContainerGateIn = (id) => {
        setLoading(true);
        axios.get(`${ipaddress}vendorInvoice/getAfterSaveData?cid=${companyid}&bid=${branchId}&val=${id}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
    ...
    const getSelectedReceiptData = (val, id, type) => {
        axios.get(`${ipaddress}vendorInvoice/getReceiptSelectedData?cid=${companyid}&bid=${branchId}&val=${val}&id=${id}&type=${type}`, {
            headers: {
                Authorization: `Bearer ${jwtToken}`
            }
        })
