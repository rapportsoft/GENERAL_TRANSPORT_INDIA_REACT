...
export default function AddBillingHeads() {
  ...
  const getPartyData = () => {
    ...
    if (flag1 === 'edit') {
      axios.get(`${ipaddress}service/getServiceData?cid=${companyid}&bid=${branchId}&serviceId=${id}`, {
        headers: {
  ...
  const updatdStatus = async () => {
    if (serviceId) {
      serviceMaster
        .updateServiceStatus(companyid, branchId, serviceId, userId, services)
        .then((res) => {
  ...
  const saveorUpdateService = (e) => {
    ...
    axios
      .post(
        `${ipaddress}service/saveService?cid=${companyid}&bid=${branchId}&flag=${flag}&user=${userId}`,
        services,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
